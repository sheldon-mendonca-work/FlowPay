...
func (c *KafkaConsumer) Start(ctx context.Context) error {
	for {
		...
		if err != nil {
			errorType := flowpayNotificationErrors.ToNotificationErrorType(err)
			logger.LogEvent(ctx, "ERROR", constants.ServiceName, "notification_execution_failed", logger.Fields{
				"topic":      msg.Topic,
				"partition":  msg.Partition,
				"offset":     msg.Offset,
			...
			if shouldCommitOnHandlerError(err) {
				...
				logger.LogEvent(ctx, "WARN", constants.ServiceName, "notification_execution_failed_committed", logger.Fields{
					"topic":      msg.Topic,
					"partition":  msg.Partition,
					"offset":     msg.Offset,
					"error_type": errorType,
				...
				logger.LogEvent(ctx, "WARN", constants.ServiceName, "notification_execution_failed_retryable", logger.Fields{
					"topic":      msg.Topic,
					"partition":  msg.Partition,
					"offset":     msg.Offset,
					"error_type": errorType,
		...
		logger.LogEvent(ctx, "INFO", constants.ServiceName, "notification_execution_succeeded", logger.Fields{
			"topic":      msg.Topic,
			"partition":  msg.Partition,
			"offset":     msg.Offset,
			"error_type": flowpayNotificationErrors.ErrorTypeNone,
...
func (c *KafkaConsumer) decodeMessage(payload []byte) (domain.PaymentTimelineEvent, error) {
	var event domain.PaymentTimelineEvent
	if err := json.Unmarshal(payload, &event); err != nil {
		return domain.PaymentTimelineEvent{}, err
	}
	return event, nil
}
...
func shouldCommitOnHandlerError(err error) bool {
	switch flowpayNotificationErrors.ToNotificationErrorType(err) {
	case flowpayNotificationErrors.ErrorTypeValidationError,
		flowpayNotificationErrors.ErrorTypeKafkaMessageDecoding:
		return true
	default:
		return false
	}
