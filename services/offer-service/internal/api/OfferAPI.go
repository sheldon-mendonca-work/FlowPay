...
func validateCreateOfferRequest(req dto.OfferCreationRequestDTO, idempotencyKey string) error {
...
func isOfferTypeValid(offer string) {
	return
}

func (h *Handler) HandleOfferIdTransactions(w http.ResponseWriter, r *http.Request) {

}

func (h *Handler) HandleOfferCodeTransactions(w http.ResponseWriter, r *http.Request) {
...
func (h *Handler) handleOfferPostMethod(w http.ResponseWriter, r *http.Request) {
	...
	var req dto.OfferCreationRequestDTO
	var serviceErr error

	defer func() {
		outcome := offerOutcome(statusCode, serviceErr)
		metrics.OfferRequestsTotal.WithLabelValues(constants.ServiceName, outcome).Inc()
		metrics.OfferRequestDuration.WithLabelValues(constants.ServiceName, outcome).Observe(time.Since(start).Seconds())
	}()
